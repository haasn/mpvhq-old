...
- Display sync state. If display sync is active (``display-sync-active``
  ...
  factor applied to achieve sync to display (``speed-correction`` property).
- Missed frames, e.g. ``M: 4``. (``vo-missed-frame-count`` property.) Shows up
  in display sync mode only. This is incremented each time a frame took longer
  to display than intended.
